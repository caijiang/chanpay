...
import me.jiangcai.chanpay.util.MD5;
...
public class ChanpayGatewayPayDemo {
    ...
    /**
     * 建立请求，以模拟远程HTTP的POST请求方式构造并获取钱包的处理结果 如果接口中没有上传文件参数，那么strParaFileName与strFilePath
     * 设置为空值 如：buildRequest("", "",sParaTemp)
     *
     * @param sParaTemp 请求参数数组
     * @return 钱包处理结果
     * @throws Exception
     */
    private static String buildRequest(Map<String, String> sParaTemp, String signType, String key, String inputCharset
        ...
        try (CloseableHttpClient client = createClient()) {
            ...
            HttpEntity entity = EntityBuilder.create()
                    .setContentEncoding(inputCharset)
                    .setParameters(generateNameValuePair(createLinkRequestParas(sPara)))
                    .build();
     ...
     */
    private static NameValuePair[] generateNameValuePair(Map<String, String> properties) throws Exception {
     ...
     * @throws Exception
     */
    private static Map<String, String> buildRequestPara(Map<String, String> sParaTemp, String signType, String key
        ...
        if ("MD5".equalsIgnoreCase(signType)) {
            signResult = buildRequestByMD5(sPara, key, inputCharset);
        } else if ("RSA".equalsIgnoreCase(signType)) {
            signResult = buildRequestByRSA(sPara, key, inputCharset);
        }
        // 签名结果与签名方式加入请求提交参数组中
        System.out.println("sign:" + signResult);
        sPara.put("sign", signResult);
     ...
     * @return 签名结果字符串
     */
    private static String buildRequestByMD5(Map<String, String> sPara, String key, String inputCharset)
            throws Exception {
        String preString = createLinkString(sPara, false); // 把数组所有元素，按照“参数=参数值”的模式用“&”字符拼接成字符串
        String signResult;
        signResult = MD5.sign(preString, key, inputCharset);
        return signResult;
    }
     ...
     * @return 签名结果字符串
     */
    private static String buildRequestByRSA(Map<String, String> sPara, String privateKey, String inputCharset)
            throws Exception {
        String preString = createLinkString(sPara, false); // 把数组所有元素，按照“参数=参数值”的模式用“&”字符拼接成字符串
        String signResult;
        signResult = RSA.sign(preString, privateKey, inputCharset);
        return signResult;
    }
     ...
     * @return 拼接后字符串
     */
    private static Map<String, String> createLinkRequestParas(Map<String, String> params) {
        Map<String, String> encodeParamsValueMap = new HashMap<>();
     ...
     */
    private void gatewayPost(Map<String, String> origMap, String charset, String MERCHANT_PRIVATE_KEY) {
        try {
